...
//uint8_t const RX_ADDRESS_P1[TX_ADR_WIDTH]  = {0xD2,0xF8,0xD0,0xD0,0xBD,0xA3,0x11,0x11}; // 定义RF收发地址1

uint8_t data rx_buf[TX_PLOAD_WIDTH];
uint8_t data tx_buf[9]={0x99};
//uint8_t data_buf[10]  = {0x00,0x01,0x00,0x01,0x00,0x04,0x03,0x02,0x01,0x00};	//写入NVN存储区域的数据
...
void main(void)
{
	...
	uint8_t  ii, j=0,  kk[2]={0,0}, tagpower[2]={0x0f,0x0f}, m=0, n=0;
	ii=0;
		
	io_init();									// I/O口初始化
	uart_init();                // 串口初始化   38400
	rf_init();									// RF初始化                            
  EA=1; 										// 允许中断                          
...
//		} 	   
		...
		/*****直接接收标签的ID信息,
		或间接通过SPI接收从读写模块SlaveReader识别到的标签ID信息*****/
        {
//		        if (spi_cs_slave==0)
//	            {  
//	               for(jj=0;jj<52;jj++);
